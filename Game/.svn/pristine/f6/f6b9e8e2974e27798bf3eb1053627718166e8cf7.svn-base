...
public class HeroVehicle : Vehicle {
	...
	public bool effectivelyDead = false;
	...
	void Start() {
		...
		_damagable.DamageCallback.Add(OnDamageCallback);
		...
		if (_config != null) {
			...
			if (IsTutorialHero()) {
				_sprite.HFlip();
				_directivePacks = new DirectivePack[]{};
			}
	...
	void Update () {
		...
		if (_damagable.Health <= 0.0f && !effectivelyDead) {
			...
			if (GameState.Instance.DidQuitApplication() == false) {
				Messenger.Broadcast("OnHeroDie", transform.position, this);
			}
		}
	}
	...
	public override Damagable GetDamagable() {
		return _damagable;
	}
	...
	void OnPooled() {
		_damagable.DamageCallback.Remove(OnDamageCallback);
	}

	bool IsTutorialHero() {
		return _config.GetHeroId().Contains("tutorial");
	}
