...
public class MasterAudioClipManager : EditorWindow {
	...
	public AudioImporterLoadType bulkLoadType = AudioImporterLoadType.CompressedInMemory;
	...
	private void ShowBulkOperations() {
		GUILayout.BeginArea(new Rect(0, 598, 895, 200));
	...
	private List<AudioInfo> FilteredClips {
		get {
			if (filterClips == null) {
				...
				filterClips.AddRange(clipList.audioInfo);
			
				filterClips.RemoveAll(delegate(AudioInfo obj) {
				...
				if (filterClips.Count > 0) {
					var isAsc = clipList.sortDir == ClipSortDirection.Ascending;
					
					filterClips.Sort(delegate(AudioInfo x, AudioInfo y) {
	...
	private void DisplayClips() {
		...
		scrollPos = GUI.BeginScrollView(new Rect(0, 107, 896, 485), scrollPos, new Rect (0, 107, 880, 25 * FilteredClips.Count + 4));	 
		...
		for (var i = 0; i < FilteredClips.Count; i++) {
			...
			var oldLoad = aClip._loadType;
			aClip._loadType = (AudioImporterLoadType) EditorGUILayout.EnumPopup(aClip._loadType, GUILayout.Width(140));
		...
		try {
			...
			for (var i = 0; i < files.Count; i++) {
				...
				if (matchingClip == null) {
					var aud = new AudioInfo(path, name, is3d, compressionBitrate, forceMono, format, loadType);
	...
	public class AudioInfo {
		...
		public AudioImporterLoadType _origLoadType;
		...
		public AudioImporterLoadType _loadType;
