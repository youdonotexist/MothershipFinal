...
public class MasterAudio : MonoBehaviour
{
	...
	public bool enableFastResponse = true;
    ...
    IEnumerator CoUpdate()
    {
        while (true)
        {
			yield return StartCoroutine(CoroutineHelper.WaitForRealSeconds(INNER_LOOP_CHECK_INTERVAL)); // works with zero timescale also.
			//yield return loopDelay; // Stops working with zero timescale.
    ...
    private static PlaySoundResult PlaySoundAtVolume(string sType,
		...
		if (_group._group.curVariationMode == MasterAudioGroup.VariationMode.Dialog) {
			if (_group._group.useDialogFadeOut) {
				FadeOutAllOfSound(_group._group.name, _group._group.dialogFadeOutTime);
			} else {
				StopAllOfSound(_group._group.name);
			}
		}
	...
	public static void CreateGroupVariationFromClip(string sType, AudioClip clip, string variationName, float volume = 1f, float pitch = 1f) {
		...
		newVar.audio.pitch = pitch;
		
		var newVariation = newVar.GetComponent<SoundGroupVariation>();
		
		var newInfo = new AudioInfo(newVariation, newVar.audio, volume);
    ...
    public static void UnpauseSoundGroup(string sType)
    {
        ...
        for (var i = 0; i < sources.Count; i++)
        {
            ...
            if (!AudioUtil.IsAudioPaused(aVar.audio)) {
                continue;
            }
	...
	public static void PauseMixer() {
        foreach (var key in audioSourcesBySoundType.Keys)
        {
            PauseSoundGroup(audioSourcesBySoundType[key]._group.name);
        }
	}
	...
	public static void UnpauseMixer() {
        foreach (var key in audioSourcesBySoundType.Keys)
        {
			UnpauseSoundGroup(audioSourcesBySoundType[key]._group.name);
        }
	}
	...
	public static void StopMixer() {
        foreach (var key in audioSourcesBySoundType.Keys)
        {
		 	StopAllOfSound(audioSourcesBySoundType[key]._group.name);
        }
	}
	#endregion
	...
	public static void ChangeBusPitch(string busName, float pitch) {
        ...
        while (sources.MoveNext())
        {
            ...
            ChangeVariationPitch(aGroup.name, true, string.Empty, pitch);
        }
	}
    ...
    /// <summary>
    /// This method will change the pitch of all clips in a Playlist.
    /// </summary>
    /// <param name="playlistName">The name of the Playlist.</param>
    /// <param name="pitch">The pitch to change the songs to.</param>
	public static void ChangePlaylistPitch(string playlistName, float pitch) {
	...
	public static void CreatePlaylist(Playlist playlist) {
        ...
        if (pl != null)
        {
	        Debug.LogError("You already have a Playlist Controller with the name '" + pl.playlistName + "'. You must name them all uniquely. Ducking and other functions will not function properly until you fix this.");
        }
	...
	public static void DeletePlaylist(string playlistName) {
		...
		for (var i = 0; i < PlaylistController.Instances.Count; i++) {
			...
			if (pc.PlaylistName == playlistName) {
				Debug.LogError("Playlist Controller '" + pc.name + "' is using Playlist '" + playlistName + "' so you cannot delete the Playlist. Please change to another Playlist before deleting.");
				return;
			}
		}
