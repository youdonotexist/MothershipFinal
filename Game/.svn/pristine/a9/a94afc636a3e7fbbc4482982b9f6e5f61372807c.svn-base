	...
	public void OnUpdatePort(int type, int level) {
		...
		if (gotone) {
			...
			if (portedMinion != null) {
				MinionPhysicsVehicle vehicle = portedMinion.GetComponent<MinionPhysicsVehicle>();
				vehicle.CleanupAndDestroy(Vector3.zero, 
				                          false, 
				                          downgrade ? MinionPhysicsVehicle.DestructionType.DOWNGRADE : MinionPhysicsVehicle.DestructionType.UPGRADE);
			}
