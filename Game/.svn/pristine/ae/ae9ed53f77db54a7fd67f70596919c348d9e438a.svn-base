...
public class VectorLine {
	...
	public bool collider {
		...
		set {
			m_collider = value;
			if (vectorObject.collider2D != null) {
				vectorObject.collider2D.enabled = value;
			}
			else if (value) {
	...
	public PhysicsMaterial2D physicsMaterial {
		...
		set {
			...
			m_vectorObject.collider2D.sharedMaterial = value;
			m_vectorObject.collider2D.enabled = false;	// Work-around for Unity bug where changing collider2D materials won't work
			m_vectorObject.collider2D.enabled = true;
		}
	}
#endif
...
//		m_mesh.MarkDynamic();	// Apparently buggy on some platforms; makes little difference in speed anyway
	...
	public void SetColor (Color color, int startIndex, int endIndex) {
		...
		int startIndex2 = Mathf.Clamp (startIndex, 0, max) * linetypeMultiplier;
		int endIndex2 = Mathf.Clamp (endIndex + 1, 1, max) * linetypeMultiplier;
	...
	private void DrawPoints () {
		...
		if (m_1pixelLine) {
			if (!m_is2D) {
				for (int i = start; i <= end; i++) {
					m_lineVertices[i] = useMatrix? cam3D.WorldToScreenPoint (thisMatrix.MultiplyPoint3x4(points3[i])) :
															cam3D.WorldToScreenPoint (points3[i]);
	...
	private void SetCollider () {
		...
		Vector3 v3;
		v3.z = -cam3D.transform.position.z;
		if (m_continuous) {
			var collider = m_vectorObject.GetComponent (typeof(EdgeCollider2D)) as EdgeCollider2D;
			var path = new Vector2[(pointsLength - 1) * 4 + 1];
			
			int end = m_lineVertices.Length;
			int startIdx = 0;
			...
			collider.pathCount = ((m_drawEnd - m_drawStart) + 1) / 2;

			int end = (m_drawEnd + 1) / 2 * 4;
			int pIdx = 0;
			for (int i = m_drawStart / 2 * 4; i < end; i += 4) {
				v3.x = m_lineVertices[i  ].x; v3.y = m_lineVertices[i  ].y;
	...
	public Vector2 GetPoint (float distance) {
		...
		Vector2 point;
		if (m_continuous) {
			point = Vector2.Lerp(points2[i-1], points2[i], Mathf.InverseLerp(m_distances[i-1], m_distances[i], distance));
		}
		else {
			point = Vector2.Lerp(points2[(i-1)*2], points2[(i-1)*2+1], Mathf.InverseLerp(m_distances[i-1], m_distances[i], distance));
		}
		if (m_useTransform) {
	...
	public Vector3 GetPoint3D (float distance) {
		...
		Vector3 point;
		if (m_continuous) {
			point = Vector3.Lerp (points3[i-1], points3[i], Mathf.InverseLerp (m_distances[i-1], m_distances[i], distance));			
		}
		else {
			point = Vector3.Lerp (points3[(i-1)*2], points3[(i-1)*2+1], Mathf.InverseLerp (m_distances[i-1], m_distances[i], distance));
		}
		if (m_useTransform) {
