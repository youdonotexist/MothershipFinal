...
public class VectorLine {
	...
	public Material material {
		...
		set {
			m_material = value;
			vectorObject.renderer.material = m_material;
		}
	}
	bool m_active = true;
	public bool active {
	...
	void SetupTriangles () {
		...
		if (m_1pixelLine) {
			...
			if (m_continuous) {
				m_triangleCount = m_isPoints? m_triangleCount = (m_pointsLength)*4 : m_triangleCount = (m_pointsLength-1)*4;
				if (m_joins == Joins.Fill) {
					m_triangleCount += (m_pointsLength-2)*4;
					// Add another join fill if the first point equals the last point (like with a square)
					if ( (m_is2D && points2[0] == points2[points2.Length-1]) || (!m_is2D && points3[0] == points3[points3.Length-1]) ) {
						m_triangleCount += 4;
						addPoint = true;
	...
	void AddEndCap () {
		...
		vectorObject.renderer.sharedMaterials = materials;
	}
	...
	void RemoveEndCap () {
		...
		materials[0] = vectorObject.renderer.materials[0];
		vectorObject.renderer.materials = materials;
	}
	...
	public static Camera SetCameraRenderTexture (RenderTexture renderTexture) {
		return SetCameraRenderTexture (renderTexture, Color.black);
	}
	...
	public static Camera SetCameraRenderTexture (RenderTexture renderTexture, Color color) {
		...
		vCam = SetCamera (CameraClearFlags.SolidColor);
	...
	bool CheckLine () {
		...
		if (m_capType != EndCap.None) {
			if (m_capType <= EndCap.Mirror) {
				...
				var d = (m_lineVertices[vIndex] - m_lineVertices[vIndex+2]).normalized * m_lineWidths[widthIndex] * 2.0f * capDictionary[m_endCap].ratio1;
			...
			if (m_capType >= EndCap.Both) {
				...
				var d = (m_lineVertices[vIndex-1] - m_lineVertices[vIndex-3]).normalized * m_lineWidths[widthIndex] * 2.0f * capDictionary[m_endCap].ratio2;
	...
	void Line2D (int start, int end, Matrix4x4 thisMatrix, bool useTransformMatrix) {
		Vector3 p1, p2;
#if !UNITY_3
		if (m_1pixelLine) {
			if (m_continuous) {
	...
	void Line3DContinuous (int start, int end, Matrix4x4 thisMatrix, bool useTransformMatrix) {
		if (!cam3D) {
	...
	void Line3DDiscrete (int start, int end, Matrix4x4 thisMatrix, bool useTransformMatrix) {
		if (!cam3D) {
			LogError("The 3D camera no longer exists...if you have changed scenes, ensure that SetCamera3D is called in order to set it up.");
			return;
		}
...
#if !UNITY_3
		if (m_1pixelLine) {
			Debug.LogWarning ("VectorLine: When using a 1 pixel line, Draw3DAuto is unnecessary. Use Draw3D instead for optimal performance.");
		}
#endif
	...
	static Vector2 GetBezierPoint (ref Vector2 anchor1, ref Vector2 control1, ref Vector2 anchor2, ref Vector2 control2, float t) {
	...
	static Vector3 GetBezierPoint3D (ref Vector3 anchor1, ref Vector3 control1, ref Vector3 anchor2, ref Vector3 control2, float t) {
		float cx = 3 * (control1.x - anchor1.x);
		float bx = 3 * (control2.x - control1.x) - cx;
		float ax = anchor2.x - anchor1.x - cx - bx;
		float cy = 3 * (control1.y - anchor1.y);
	...
	static Vector2 GetSplinePoint (ref Vector2 p0, ref Vector2 p1, ref Vector2 p2, ref Vector2 p3, float t) {
		float t2 = t*t;
		float t3 = t2*t;
	...
	static Vector3 GetSplinePoint3D (ref Vector3 p0, ref Vector3 p1, ref Vector3 p2, ref Vector3 p3, float t) {
		float t2 = t*t;
		float t3 = t2*t;
