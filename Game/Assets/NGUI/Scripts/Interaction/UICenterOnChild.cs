...
public class UICenterOnChild : MonoBehaviour
{
	...
	void Start () { Recenter(false); }
	void OnEnable () { if (mScrollView) { mScrollView.centerOnChild = this; Recenter(true); } }
	void OnDisable () { if (mScrollView) mScrollView.centerOnChild = null; }
	void OnDragFinished () { if (enabled) Recenter(true); }
	...
	[ContextMenu("Execute")]
	public void Recenter (bool animate)
	{
		if (mScrollView == null)
		{
		...
		CenterOn(closest, panelCenter, animate);
	}
	...
	void CenterOn (Transform target, Vector3 panelCenter, bool animate)
	{
		if (target != null && mScrollView != null && mScrollView.panel != null)
		{
...
#endif
			{
				SpringPanel.Begin(mScrollView.panel.cachedGameObject,
				                  panelTrans.localPosition - localOffset, animate ? springStrength : 10000).onFinished = onFinished;
	...
	public void CenterOn (Transform target, bool animate)
	{
		if (mScrollView != null && mScrollView.panel != null)
		{
			Vector3[] corners = mScrollView.panel.worldCorners;
			Vector3 panelCenter = (corners[2] + corners[0]) * 0.5f;
			CenterOn(target, panelCenter, animate);
		}
	}
}
