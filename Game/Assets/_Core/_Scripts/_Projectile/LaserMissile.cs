...
public class LaserMissile : MonoBehaviour
{
	...
	public void StartFiring(float cd) {
		...
		if (chargeAnimationParent == null) {
			chargeAnimationParent = (GameObject)Instantiate (chargeAnimationPrefab, transform.position, chargeAnimationPrefab.transform.rotation);
		...
		if (didHit) {
			...
			if (dam != null) {
				...
				if (chargeExplosion == null) {
					...
					point.z -= 2.0f;
					chargeExplosion = (GameObject)Instantiate (chargeExplosionPrefab, point, chargeExplosionPrefab.transform.rotation);	
