...
public class ScienceHeroDirectives : DirectivePack
{
	...
	IEnumerator FireMissile(AIDirective directive) {
		...
		if (_vehicle != null) {
			SpatialNode[] heroNodes = _aiManager.GetHeroNodes();
			SpatialNode chosenNode = null;
			int randomStart = Random.Range(0, heroNodes.Length);
			int i = randomStart;

			LayerMask avoid = LayerMaskHelper.OnlyIncluding(new int[]{LayerMask.NameToLayer("HeroMine"), LayerMask.NameToLayer("HeroShield")});
			do {
				SpatialNode startNode = heroNodes[i];
				RaycastHit[] hits = Physics.RaycastAll(startNode.transform.position, 
				                   (_mothership.transform.position - startNode.transform.position).normalized, 
				                   Mathf.Infinity, 
				                   avoid);
				if (hits.Length == 0) {
			...
			if (chosenNode != null) {
				if (_vehicle != null) {
					//First move to the open node
					Vector3 startPosition = _vehicle.transform.position;
					float startDistance = Vector3.Distance(startPosition, chosenNode.transform.position);
					float moveTime = startDistance / _vehicle._config.getInt ("moveSpeed");
					
					yield return StartCoroutine(MoveToLaneProper(chosenNode, moveTime));
					
					//Then FIRE LE MISSILE
					if (_vehicle != null) {
						//Vector3 direction = (_mothership.transform.position - chosenNode.transform.position).normalized;
						Vector3[] path = new Vector3[] {chosenNode.transform.position, chosenNode.transform.position + (_vehicle.transform.right * 2000.0f)};
						if (_vehicle != null) {
							...
							float missileSpeed = _vehicle._config.getFloat("empMissileSpeed");
							hm.SetPathNormal(path);
							Debug.Log("Fire missile Speed: " + missileSpeed);
							hm.speed = (missileSpeed < 300.0f ? 300.0f : missileSpeed);
						}
