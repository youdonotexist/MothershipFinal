...
Ext.define('MothershipEditor.controller.HeroListController', {
	...
	config: {
		...
		control: {
            ...
            'addherobasicworkflow selectfield': {
                change: 'onHeroTypeChange'
            },
	...
	onWorkflowViewWillAppear: function(view, created) {
		if (view.xtype == 'addherobasicworkflow') {
			...
			var workflowController = MothershipEditor.app.getController('WorkflowController'),
                ...
                selectField = view.down('selectfield');
    ...
    onHeroTypeChange: function(select, newValue, oldValue) {
        if (MothershipEditor.CurrentBattle.get('hero_type_id') != "MIXED") return;

        var workflowController = MothershipEditor.app.getController('WorkflowController'),
            hero = workflowController.getWorkflowData('HeroBuilder');

        console.log("Doing this in the hero type change section");
        console.log(hero);
        console.log(newValue);

        hero.set('hero_type_id', newValue);
    },
